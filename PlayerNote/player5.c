...
void video_refresh_timer(void *userdata) {
  ...
  if(is->video_st) {
    if(is->pictq_size == 0) {
      ...
      if(delay <= 0 || delay >= 1.0) {//小于0，或者大于1秒，判定为错误
	/* if incorrect delay, use previous one */
	delay = is->frame_last_delay;//设置为上一帧的delay，设置一个默认值
      }
      /* save for next time */
      // 更新delay和pts
	...
	if(diff <= -sync_threshold) {//视频时间是在音频时间之前，需要快播
	  delay = 0;
	} else if(diff >= sync_threshold) {//视频和音频的阈值diff >= 我们设置的阈值sync_threshold，代表视频要展示的时间远远未到，这时等待时间加长，*2倍
	  delay = 2 * delay;
	}
      }
      //系统时间+delay
